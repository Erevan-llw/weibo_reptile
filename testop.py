...
# 今日头条api链接
t_url = 'https://www.toutiao.com'  # 今日头条头条内容页面链接头
t_headers = {
    'user-agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_12_3) AppleWebKit/537.36 (KHTML, like Gecko) '
                  'Chrome/71.0.3578.98 Safari/537.36 '
}  # 今日头条请求头
t_cookies = {'tt_webid': '6649949084894053895'}  # 此处cookies可从浏览器中查找，为了避免被头条禁止爬虫
...
def existential_status(query, user):
    """判断话题存在状态"""
    status = 'new'
    list_q = [query, user]
    # print(last_register_list)
    for list_r in last_register_list:
        if list_r[1] == 'False':
            list_r[1] = False
        if list_q == list_r:
            status = 'update'
    ...
    if len(register_list) != 0:
        for list_e in register_list:
            if list_q == list_e:
                continue
            else:
                register_list.append(list_q)
                print("a new guy!>>>")
                return status
    else:
        register_list.append(list_q)
    ...
    if choose == 'y':
        ...
        with open('l_query_list.csv', mode='r', newline='') as f:
            ...
            for row in csv_reader:
                query_r = [row[0], row[1]]
                last_register_list.append(query_r)
            print('暂存文件已读入！')
            print(last_register_list)
...
def message_recode():
    """敏感信息录入函数"""
    # print(q)
    status = existential_status(q, data_dic['user'])
...
while True:
    ...
    for q in qlist:
        ...
        if select_q(q):
            ...
            # 页面二获取数据
            data_con = tag_data(q)
            if jsonpath(data_con, '$.data.baseInfo.count.read') and data_con:
        ...
        count += 1
    print(f'本次运行共传输{succeed_count}条”新浪微博“舆情...')
    print('正在爬取今日头条相关信息...')

    print(register_list)
    print(len(register_list))
    print(last_register_list)
    print(len(last_register_list))
    temp_register_list = register_list + last_register_list
    print(len(temp_register_list))
