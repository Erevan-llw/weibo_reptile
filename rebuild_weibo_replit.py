...
l_keywords = ['河北', '山西', '辽宁', '吉林', '黑龙江', '江苏', '浙江', '安徽', '福建', '江西', '山东', '河南', '湖北', '湖南', '广东', '海南', '四川',
...
register_list = []  # 缓存tag列表
last_register_list = []  # 一组更新后的缓存tag列表
temp_list = []  # 暂存tag列表
url_list = []  # 暂存链接列表
result_query_list = [[], [], [], [], [], [], [], []]  # 多次暂存结果列表

sleep_time = 600  # 休眠时间设置
...
def get_sup_sensitive():
    """获取服务器端专属敏感词"""
    sup_list = [{'username': 'Erevan', 'dictionary': '疫情,拐卖,被拐,偷渡'}]
...
while True:
    for l_keyword in l_keywords:
        while page <= l_page:
            ...
            page += 1
        print(f'\n已爬取{l_keyword}分区前{l_page}页信息，本次进程已爬取{len(url_list)}条热门信息')
        page = 1
    # print(url_list)
